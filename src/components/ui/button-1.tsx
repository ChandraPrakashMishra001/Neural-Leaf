    ...
    <div className={`relative inline-block group dark:bg-black bg-white dark:border-white border-black border-2 rounded-full ${className}`}>
      ...
      <div className="relative w-full h-full overflow-hidden rounded-full" style={{ WebkitMaskImage: '-webkit-radial-gradient(white, black)', transform: 'translateZ(0)' }}>
        ...
        {[1, 2, 3, 4, 5].map((i) => (
          <span
            key={i}
            className={`absolute inset-0 rounded-full border-solid border-[3px] border-white/20 mix-blend-overlay filter ${i <= 2 ? 'blur-[3px]' : i === 3 ? 'blur-[5px]' : 'blur-[4px]'}`}
          ></span>
        ))}
      ...
      <button
        className="absolute inset-0 rounded-full bg-transparent cursor-pointer w-full h-full flex items-center justify-center"
        type="button"
        onClick={onClick}
        onMouseEnter={() => setIsHovered(true)}
        onMouseLeave={() => setIsHovered(false)}>
        <span className="flex items-center justify-center px-4 gap-2 rounded-full text-white text-sm font-semibold tracking-wide whitespace-nowrap">
          {children}
        </span>
      </button>
    </div>
